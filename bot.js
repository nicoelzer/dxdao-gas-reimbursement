...
async function fetchGasSpenings() {
  try {
    ...
    console.log(
      `Found ${genesisVotes.events.length} vote, ${genesisStakes.events.length} staking & ${genesisProposalCreations.events.length} proposalCreation transactions on Genesis Protocol `
    );

    for (var i in genesisVotes.events) {
      var receipt = await web3.eth.getTransactionReceipt(
        genesisVotes.events[i].transactionHash
      );
      ...
      upsertAccountGasSpending(
        { id: genesisVotes.events[i].returnValues._voter },
        { id: genesisVotes.events[i].returnValues._voter }
      );
    ...
    for (var i in genesisStakes.events) {
      var receipt = await web3.eth.getTransactionReceipt(
        genesisStakes.events[i].transactionHash
      );
      var tx = await web3.eth.getTransaction(
        genesisStakes.events[i].transactionHash
      );
      ...
      upsertAccountGasSpending(
        { id: genesisStakes.events[i].returnValues._staker },
        { id: genesisStakes.events[i].returnValues._staker }
      );
    ...
    for (var i in genesisProposalCreations.events) {
      var receipt = await web3.eth.getTransactionReceipt(
        genesisProposalCreations.events[i].transactionHash
      );
      var tx = await web3.eth.getTransaction(
        genesisProposalCreations.events[i].transactionHash
      );
      var block = await web3.eth.getBlock(
        genesisProposalCreations.events[i].blockNumber
      );
      ...
      if (receipt.status) {
        upsertGasSpending(
          { id: genesisProposalCreations.events[i].transactionHash },
          {
            id: genesisProposalCreations.events[i].transactionHash,
            proposalId:
              genesisProposalCreations.events[i].returnValues._proposalId,
    ...
    for (var j in scheme) {
      ...
      let avatarFilter = { "_avatar:": process.env.AVATAR_ADDRESS };
      ...
      if (scheme[j].votingMachineAddress) {
        ...
        console.log(
          `Found ${votes.events.length} vote, ${stakes.events.length} staking & ${proposalCreations.events.length} proposalCreation transactions on ${scheme[j].name} `
        );
